...
def _format_distribution(distribution: str, other: Optional[Dict] = None) -> str:
    ...
    if distribution == "dgnorm":
        ...
        return f"Generalised Normal with shape={shape:.4f}" if isinstance(shape, float) else f"Generalised Normal with shape={shape}"
    elif distribution == "dt":
        nu = other.get("nu", "?")
        return f"Student t with df={nu:.4f}" if isinstance(nu, float) else f"Student t with df={nu}"
    elif distribution == "dalaplace":
        alpha = other.get("alpha", "?")
        return f"Asymmetric Laplace with alpha={alpha:.4f}" if isinstance(alpha, float) else f"Asymmetric Laplace with alpha={alpha}"
    elif distribution == "dlgnorm":
        shape = other.get("shape", "?")
        return f"Log-Generalised Normal with shape={shape:.4f}" if isinstance(shape, float) else f"Log-Generalised Normal with shape={shape}"
...
def _get_persistence_from_model(model: Any) -> Dict[str, Any]:
    ...
    if (hasattr(model, 'adam_estimated') and model.adam_estimated and
        hasattr(model, 'adam_created') and model.adam_created):
...
def _format_persistence_vector(
    model: Any,
    digits: int = 4
) -> str:
    """
    ...
    value_strs = [f"{v:{max(len(names[i]), digits+2)}.{digits}f}" for i, v in enumerate(values)]
            ...
            if order > 0:
                ...
                for j in range(order):
                    if param_idx < len(ar_params):
                        lines.append(f"  AR({j+1}) Lag {lag}: {ar_params[param_idx]:.{digits}f}")
                        param_idx += 1
            ...
            if order > 0:
                ...
                for j in range(order):
                    if param_idx < len(ma_params):
                        lines.append(f"  MA({j+1}) Lag {lag}: {ma_params[param_idx]:.{digits}f}")
                        param_idx += 1
...
def _compute_forecast_errors(
    ...
    asymmetry = (pos_errors - neg_errors) / (pos_errors + neg_errors) if (pos_errors + neg_errors) != 0 else 0
    ...
    if len(y_in_sample) > period:
        ...
        rmsse = rmse / np.sqrt(np.mean(naive_errors ** 2)) if np.mean(naive_errors ** 2) != 0 else np.nan
    else:
        mase = np.nan
        rmsse = np.nan
...
def model_summary(model: Any, digits: int = 4) -> str:
    ...
    if _can_compute_ic(model):
        ...
        lines.append("Information criteria are unavailable for the chosen loss & distribution.")
...
def _get_distribution(model: Any) -> str:
    ...
    if hasattr(model, 'general') and model.general:
        return model.general.get('distribution_new', model.general.get('distribution', 'dnorm'))
...
def _can_compute_ic(model: Any) -> bool:
    ...
    if hasattr(model, 'general') and model.general:
        loss = model.general.get('loss', 'likelihood')
        distribution = model.general.get('distribution_new', model.general.get('distribution', 'dnorm'))
