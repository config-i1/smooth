...
#' \item \code{ot} - The value of the binary occurrence variable;
...
sim.oes <- function(model="MNN", obs=10, nsim=1, frequency=1,
    ...
    model$ot <- model$probability;
    model$ot[] <- rbinom(length(model$probability),1,model$probability);
    if(nsim==1){
        model$logLik <- sum(log(model$probability[model$ot==1])) + sum(log(1-model$probability[model$ot==0]));
    }
    else{
        model$logLik <- vector("numeric",nsim);
        for(i in 1:nsim){
            model$logLik[i] <- sum(log(model$probability[model$ot[,i]==1,i])) + sum(log(1-model$probability[model$ot[,i]==0,i]));
        }
    }
