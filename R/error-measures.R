...
GMRAE <-function(actual,forecast,benchmark,round=3){
...
# benchmark - forecasted or fitted values of etalon method.
    if((length(actual) != length(forecast)) | (length(actual) != length(benchmark)) | (length(benchmark) != length(forecast))){
        ...
        return(round(exp(mean(log(abs(actual-forecast)/abs(actual-benchmark)),na.rm=TRUE)),round));
    }
}

TRAE <-function(actual,forecast,benchmark,round=3){
...
# benchmark - forecasted or fitted values of etalon method.
    if((length(actual) != length(forecast)) | (length(actual) != length(benchmark)) | (length(benchmark) != length(forecast))){
        ...
        return(round(1-atan(abs(actual-forecast)/abs(actual-benchmark))/(pi/4),round));
    }
}