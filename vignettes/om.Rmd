...
External regressors are incorporated via `cbind()`, exactly as in `adam()`. The first column of the matrix is treated as the response; subsequent columns become regressors. Here we include a simple indicator for the high-demand regime (last 60 observations):

```{r regression}
highPeriod <- ts(c(rep(0, 60), rep(1, 60)))
yReg <- cbind(y, highPeriod)
omReg <- om(yReg, model="NNN", constant=TRUE,
