---
title: "Advanced Dynamic Adaptive Model"
...
testModel <- adam(xreg, "NNN", h=18, silent=TRUE, holdout=TRUE, orders=c(0,1,1),
                  initial=testModel$initial, arma=testModel$arma)
testModel
names(testModel$initial)[1] <- names(testModel$initial)[[1]] <- "level"
testModel2 <- adam(xreg, "ANN", h=18, silent=TRUE, holdout=TRUE,
...
```{r}
testModel <- adam(xreg, "AAN", h=18, silent=FALSE, holdout=TRUE, orders=c(1,0,1))
summary(testModel)
```
...
testModel <- auto.adam(M3[[1234]], "AAN", orders=list(ar=2,i=2,ma=2), silent=TRUE,
                       distribution=c("dnorm","dlogis","dlaplace","ds","dgnorm"))
testModel
```
...
If you specify `outliers="select"`, the function will create leads and lags 1 of the outliers and then select the most appropriate ones via the `xregDo` parameter of adam.
