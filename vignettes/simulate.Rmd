---
title: "simulate(), sim.es(), sim.ssarima(), sim.ces() - simulate functions for ETS, SARIMA and CES"
author: "Ivan Svetunkov"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{simulate(), sim.es(), sim.ssarima(), sim.ces() - simulate functions for ETS, SARIMA and CES}
...
## SARIMA
Let's start from something simple. By default `sim.ssarima()` generates data from ARIMA(0,1,1). Let's produce 10 time serie with 120 observations of that:
...
Finally, the most complicated CES model is the one with "full" seasonality, implyin that there are two complex exponential smoothing models inside: one for seasonal and the other for non-seasonal part:
```{r sim_ces_(f)}
...
Note that GES is still an ongoing research and its properties are currently understudied.
...
ourModel <- ves(x$data,model="AAN",persistence="dependant")
Y <- simulate(ourModel)
plot(Y)
```
